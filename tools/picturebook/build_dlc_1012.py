# -*- coding: utf-8 -*-
"""TGAA1-DLC-1.0.12 = released 1.0.10 + the English Picture Book / Editor's Notes / theme pages.

Same as build_dlc_1011.py, rebuilt from the same 1.0.10 base after the see-through pages were redone
(ERASE=see, user review 2026-09-15). Built from the 1.0.10 CIA, not from 1.0.11, so the verification
still diffs against the extraction we trust.

    python build_dlc_1012.py
...
2. the Episode 0 cover carries the version stamp: first prove stamp_dlc_covers' method reproduces
   the shipped 1.0.10 cover byte for byte, then stamp "DLC 1.0.12" on the LaMa-erased cover source
   (cover_build/simple_out_lama/aoc00.png); issues 9-13 covers from cover_build/ph_out_lama (LaMa);
3. build.py rebuilds only the changed contents (encrypted CFA route) with TMD version 3.0.12
   (the project rule: DLC TMDs are major 3; 1.0.10 shipped as 3.0.10, 1.0.11 as 3.0.11);
...
SHELL = os.path.join(ROOT, 'Final', '_superseded', 'v1.6', 'TGAA1-DLC-1.0.10.cia')   # _CURRENT now holds 1.0.11
OUT = os.path.join(ROOT, 'Final', '_new', 'TGAA1-DLC-1.0.12.cia')
...
def main():
    ...
    # stamp: prove the method, then stamp 1.0.12
    ...
    # 1.0.12 cover: the LaMa-erased Episode 0 source (simple.py COVER_ERASE=lama, user OK 2026-09-15)
    new_cover = stamped_cover('DLC 1.0.12', os.path.join(COVERS, 'simple_out_lama', 'aoc00.png'))
    open(os.path.join(WORK, cover_rel.replace('/', os.sep)), 'wb').write(new_cover)
    written[cover_rel] = hashlib.sha256(new_cover).hexdigest()
    # build
    args = [sys.executable, os.path.join(ROOT, 'testimony_pipeline', 'build.py'), SHELL, OUT, '3.0.12']   # TMD major 3 like every DLC build (1.0.10 = 3.0.10); stamp/filename 1.0.12
    ...
    name = 'TGAA1-DLC-1.0.12-candidate'
